/* --- Packaging Logic --- */
function shouldPackage(item, itemMap, targetX, targetY) {
    // Only regular items can enter an existing package
    if (item.isPackaged) return null;

    // Check if item can merge with something at its intended destination
    if (!isValid(targetX, targetY)) return null;
