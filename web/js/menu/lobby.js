...
const Lobby = {
    ...
    // Player icons for representation
    playerIcons: [
        'fa-user',
        'fa-user-astronaut',
        'fa-user-ninja',
    ...
    getPlayerIcon(playerIndex) {
        return this.playerIcons[playerIndex % this.playerIcons.length];
    },
    ...
    update() {
        ...
        state.players.forEach((player, index) => {
            ...
            playerIcon.innerHTML = this.getPlayerDisplayIcon(player, index);
    ...
    updateRemoteCursor(playerId, x, y) {
        ...
        if (!cursor) {
            ...
            icon.innerHTML = this.getPlayerDisplayIcon(player, playerIndex >= 0 ? playerIndex : 0);
        ...
        if (label) {
            const icon = label.querySelector('.cursor-icon');
            if (icon) {
                icon.innerHTML = this.getPlayerDisplayIcon(player, playerIndex >= 0 ? playerIndex : 0);
            }
    ...
    refreshMoneyDisplay() {
        ...
        const connectedPlayers = otherPlayers;
        ...
        if (connectedPlayers.length === 0) {
            ...
            connectedPlayers.forEach((player, index) => {
                ...
                const playerIcon = document.createElement('div');
                playerIcon.className = 'sidebar-player-icon';
                const playerIndex = state.players.findIndex(p => p.id === player.id);
                playerIcon.innerHTML = this.getPlayerDisplayIcon(player, playerIndex >= 0 ? playerIndex : index);
