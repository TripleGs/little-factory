...
function countProducersByType(producerTypeId) {
    let count = 0;
    for (let y = 0; y < state.rows; y++) {
        for (let x = 0; x < state.cols; x++) {
            const tile = state.grid[y][x];
            if (tile && tile.type === 'producer' && tile.producerType === producerTypeId) {
                count++;
            }
        }
    }
...
function getToolCost(toolId, options = {}) {
    ...
    if (toolId === 'producer') {
        const producerTypeId = options.producerTypeId ?? state.selectedProducerType ?? 0;
        const existingCount = countProducersByType(producerTypeId);
